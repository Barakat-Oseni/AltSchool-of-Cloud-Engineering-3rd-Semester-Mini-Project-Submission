...
resource "aws_lb_target_group" "terraform-target-group-barakat" {
  name     = "target-group-terra-barakat"
  port     = 80
  protocol = "HTTP"
  vpc_id   = aws_vpc.trial-vpc.id
...
resource "aws_lb_listener" "terraform-listener-barakat" {
  ...
  default_action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.terraform-target-group-barakat.arn
  }
}

# Create the listener rule

resource "aws_lb_listener_rule" "terraform-listener-rule-barakat" {
  listener_arn = aws_lb_listener.terraform-listener-barakat.arn
  priority     = 1
